...
const resp = PL.buildMarketingIntelligenceResponse({ message: '2024년과 2025년 1월부터 6월까지의 월별 매출을 그래프로 보여줘', orders: u.orders, products, nowMs });
const entry = MEM.createMarketingAnalysisMemoryEntry({ question: '2024년과 2025년 1월부터 6월까지의 월별 매출을 그래프로 보여줘', plan: resp.plan, artifact: resp.artifact, resultType: resp.result.narrative.answerType, nowMs });
...
const similar = MEM.findSimilarMarketingAnalysisMemories({ question: '2024년과 2025년 상반기 월별 매출 비교', plan: resp.plan, limit: 5 });
...
const conv = PL.buildMarketingIntelligenceResponse({ message: '2025년 신규 가입회원의 구매전환율을 알려줘', orders: u.orders, products, nowMs });
