...
// RC-2 D-1.3: 확인 계열 결정(확인 완료·수정 요청·미채택)은 **제출된 결과**가 있어야 성립한다.
//   이 하네스의 관심사는 지휘 권한이므로 단언은 그대로 두고 전제만 실제 경로(배정 → 제출)로 맞춘다.
// AI 가 수행자일 때의 제출자는 그 AI 본인이다(D-1.3 W27).
const AI_INVENTORY = { kind: 'agent', teamId: 'product', label: '재고 감시 AI', agentId: 'inventory_monitor' };

const submitted = (taskId, teamId = 'product') => {
  const lead = A.actorForRole(teamId);
  A.assignExecutor(taskId, { kind: 'human', actor: lead }, { nowIso: AT });
  A.submitResult(taskId, { resultSummary: '업무보고(검사 전제)', actor: lead }, { nowIso: AT });
};
...
red('P3. AI 결과가 담당 팀장 확인 없이 HQ 승인함으로 직행하지 않는다',
  (() => { reset();
    A.acceptRuntimeProposals({
      ...
      // D-1.3: 결과라고 주장하려면 실제 근거가 있어야 한다.
      proposedApprovalItems: [{ taskId: 'T1', correlationId: 'C1', title: 'AI 결과', agentId: 'inventory_monitor', artifactRefs: ['art-inv-1'] }]
...
red('P25. 결과물 없이 제출·승인 진입이 불가하다',
  (() => { if (!hasFn('submitResult')) return false;
    ...
    const empty = A.submitResult(t.ref.taskId, { artifactRefs: [], actor: AI_INVENTORY }, { nowIso: AT });
    const withRef = A.submitResult(t.ref.taskId, { artifactRefs: ['art-1'], actor: AI_INVENTORY }, { nowIso: AT });
...
red('P26. 결과 제출 후에만 담당 팀장 확인 대기에 표시된다',
  (() => { if (!hasFn('submitResult')) return false;
    ...
    A.submitResult(t.ref.taskId, { artifactRefs: ['art-1'], actor: AI_INVENTORY }, { nowIso: AT });
...
red('P31. 인간 인수 시 기존 AI 시도·결과·인수 사유가 보존된다',
  (() => { if (!hasFn('assignExecutor')) return false;
    reset();
    // (a) 수행 중인 AI 작업을 팀장이 그대로 인수 — 기존 시도와 사유가 이력에 남는다.
    const t = directive();
    A.assignExecutor(t.ref.taskId, { kind: 'agent', executorId: 'inventory_monitor', actor: LEAD_PRODUCT }, { nowIso: AT });
    A.takeOverByLead(t.ref.taskId, { actor: LEAD_PRODUCT, reason: 'AI 진행이 늦어 직접 처리' }, { nowIso: AT });
    const took = S.loadLifecycleTasks()[0];
    const hist = took.executorHistory ?? [];
    const takeoverOk = hist.length >= 2 && hist[0].kind === 'agent' && hist[0].id === 'inventory_monitor'
      && hist[hist.length - 1].kind === 'human'
      && hist.some((h) => h.reason && h.reason.includes('직접 처리'))
      && took.executorKind === 'human' && took.status === 'in_progress';

    // (b) AI 결과가 이미 제출된 뒤에는 조용히 갈아끼우지 않는다.
    //     수정 요청으로 새 버전이 열리고, 원본의 AI 결과·사유는 그대로 남는다.
    reset();
    const t2 = directive();
    A.assignExecutor(t2.ref.taskId, { kind: 'agent', executorId: 'inventory_monitor', actor: LEAD_PRODUCT }, { nowIso: AT });
    A.submitResult(t2.ref.taskId, { artifactRefs: ['art-ai-1'], actor: AI_INVENTORY }, { nowIso: AT });
    const silentSwap = A.assignExecutor(t2.ref.taskId, { kind: 'human', actor: LEAD_PRODUCT, reason: '직접 처리' }, { nowIso: AT });
    A.applyDecision(t2.ref.taskId, { kind: 'approve', actor: LEAD_PRODUCT }, { nowIso: AT });
    A.applyDecision(t2.ref.taskId, { kind: 'request_revision', actor: HQ, reason: 'AI 결과 미흡해 직접 처리' }, { nowIso: AT, newId: ids.newId });
    const all = S.loadLifecycleTasks();
    const orig = all.find((x) => x.ref.taskId === t2.ref.taskId);
    const rev = all.find((x) => x.ref.revisionOfTaskId === t2.ref.taskId);
    const retake = rev && A.assignExecutor(rev.ref.taskId, { kind: 'human', actor: LEAD_PRODUCT, reason: '이번엔 직접 처리' }, { nowIso: AT });
    const revSaved = S.loadLifecycleTasks().find((x) => x.ref.revisionOfTaskId === t2.ref.taskId);

    return takeoverOk
      && silentSwap.ok === false                                   // 제출된 결과를 조용히 갈아끼우지 않는다
      && (orig.artifactRefs ?? []).includes('art-ai-1')            // AI 결과 보존
      && JSON.stringify(orig).includes('AI 결과 미흡해 직접 처리') // 사유 보존
      && orig.executorId === 'inventory_monitor'                   // 누가 했었는지 보존
      && !!retake && retake.ok === true && revSaved.executorKind === 'human';
  })(), noFn('assignExecutor'), '진행 중 인수 보존 + 제출 후 무단 교체 차단 + 수정본 재선택');
...
red('A26R. HQ→상품팀장 지시 → 팀장이 수행자 선택 → 결과 제출 (담당팀/경로 정상)',
  (() => { if (!hasFn('createDirectiveTask')) return false;
    ...
    const sub = A.submitResult(t.ref.taskId, { artifactRefs: ['art-1'], actor: AI_INVENTORY }, { nowIso: AT });
