...
// GREEN F.2: "데이터 기준" 문장만 보면 다른 문장(주문 구성 등)의 오표시를 놓친다.
//   사용자에게 전달되는 것은 facts 전체이므로 **전체를 합쳐** 검사한다.
const factsAllFixture = (factsFixture?.facts ?? []).map(String).join(' ');
...
// GREEN F.2 — facts **전체**에서 실제 오인 표현이 없어야 한다(문장 하나만 고쳐선 안 된다).
const REAL_MISREAD = /실제 데이터|REAL|실제 주문|실 \d+\s*건|실 \d+\s*\+/;
const misread = factsAllFixture.match(REAL_MISREAD);
red('G1b. fixture 전체 facts 에 실제 오인 표현이 없다 (실 N건 / 실 N + / 실제 주문 / REAL / 실제 데이터)',
  !misread,
  `잔존="${misread ? misread[0] : ''}" · facts="${factsAllFixture.slice(0, 120)}…"`,
  '전체 facts 에 실제 오인 표현 0건');
red('G2b. fixture 전체 facts 가 시험 데이터임을 밝힌다',
  /시험 데이터|기능시험/.test(factsAllFixture),
  `facts="${factsAllFixture.slice(0, 120)}…"`, '"시험 데이터/기능시험" 포함');
...
// GREEN F.2: 실제 성공 0건은 계약상 '실제 데이터 0건' → sourceMode 는 정확히 'real' 이어야 한다.
//   (이전 검사는 `!== 'synthetic'` 이라 unavailable 도 통과시켰다 — 느슨한 조건 금지.)
red('G5. SourceOfTruth: 실제 성공 0건 → sourceMode 가 정확히 real',
  sotReal.sourceMode === 'real', `sourceMode=${sotReal.sourceMode}`, "sourceMode='real'");
red('G5b. SourceOfTruth 무회귀: 주문실패+시뮬=synthetic · 전부실패=unavailable',
  sotFail.sourceMode === 'synthetic' && sotNone.sourceMode === 'unavailable',
  `주문실패+시뮬=${sotFail.sourceMode} · 전부실패=${sotNone.sourceMode}`,
  `${sotFail.sourceMode} / ${sotNone.sourceMode}`);
