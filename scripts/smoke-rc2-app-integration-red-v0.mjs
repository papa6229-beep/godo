...
red('A1. 저장된 lifecycle task 가 첫 hydration 에서 App 업무 상태로 복원된다',
  (() => { if (!A) return false;
    ...
    A.submitResult(t.ref.taskId, { artifactRefs: ['art-inv-1'], actor: LEAD_PRODUCT }, { nowIso: AT });
    const st = reload();
...
// RC-2 D-1.2: 업무는 생성만으로 승인 대기가 되지 않는다(수신≠승인).
//   아래 검사들의 전제(= 확인 대기 N건)를 실제 경로로 만들어 준다.
