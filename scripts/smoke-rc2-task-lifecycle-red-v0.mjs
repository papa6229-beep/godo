...
red('R2a. 승인 대기(pending) 기록이 추적 식별자(taskId 우선·refId 후퇴)를 갖는다',
  !!pendingEv && typeof (pendingEv.taskId ?? pendingEv.refId) === 'string' && String(pendingEv.taskId ?? pendingEv.refId).length > 0,
...
// 계약: 제안 agentId 는 **런타임 canonical 네임스페이스**여야 한다(화면/레거시 id 금지).
//   팀장 id(cs_lead 등)도 런타임 canonical 이므로 정상이다. 판정 기준은 별칭표다.
const orchAgentIds = [...orch.proposedTasks.map((t) => t.agentId), ...orch.proposedApprovalItems.map((i) => i.agentId)];
const legacyLeak = OPT.IDREG ? orchAgentIds.filter((id) => OPT.IDREG.toCanonicalAgentId(id) !== id) : orchAgentIds;
red('R5a. 제안 항목의 agentId 가 런타임 canonical 네임스페이스로 정규화돼 있다',
  legacyLeak.length === 0,
  `레거시 표기 잔존=[${[...new Set(legacyLeak)].join(', ')}]`,
  `canonical=[${[...new Set(orchAgentIds)].join(', ')}]`);
...
red('P1. HQ 지시 업무: 담당팀 완료 보고 후 HQ 가 최종 확인한다',
  ...
  })(), noMod('taskLifecycleContract'), '계약 충족');
...
red('P15. 업무·승인·결정 이력이 새로고침 후에도 보존된다(schemaVersion 보유·바이너리 미저장)',
  ...
  })(), noMod('taskLifecycleStore'), '계약 충족');
