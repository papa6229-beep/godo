...
const SYSTEM = [
  ...
  '- 원본 "메인섹션"은 **메인 이미지 + 요약정보 + 원본의 3줄 요약**으로 끝난다. 그 이상 넓히지 말 것.',
  '  · 실제 판매용 **패키지 박스 이미지**가 그 3줄 요약 **바로 위 또는 바로 아래**에 붙어 있을 때만,',
  '    그 패키지 박스까지 메인섹션에 포함한다.',
  '  · 파우치·케이블·구성품을 함께 찍은 사진·제품 특징 사진은 **패키지가 아니다**(메인섹션에 넣지 말 것).',
  '- bodyStartIndex = **그 메인섹션이 끝난 바로 다음에 처음 시작되는 본문 밴드 번호**.',
  '  · `제품특징`·`제품 포인트`·`Point`·번호·기능 설명·사용 설명·구성품 소개가 시작되면 **그 지점이 본문이다.**',
  '    여백이 없거나 이미지와 글자가 겹쳐 보여도 **본문으로 넘긴다.**',
  '  · "옵션"이라는 넓은 표현으로 본문 시작을 **뒤로 미루지 말 것.**',
