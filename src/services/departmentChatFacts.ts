  ...
  if (team === 'cs') {
    ...
    return {
      ...
      answerGuidance:
        ...
        // ── CS Draft Composer 연결(답변 초안 요청 시) ──
        '[답변 초안] 사용자가 "답변 초안/답장/고객에게 보낼 답변을 작성"해 달라고 하면, 위 [연결 주문 facts]와 Evidence Policy를 근거로 "고객 발송용 초안 1개"만 짧고 자연스럽게 작성하라. ' +
        '초안은 "아래처럼 답변하시면 됩니다." 다음에 인사~감사 인사로 끝나는 종결형으로 쓰고, [고객 발송용 초안]/[내부 확인 메모]처럼 두 블록으로 나누지 마라. ' +
        '근거 요약/missingData/PG 승인번호/transactionId 같은 내부 필드명을 고객 초안에 넣지 마라. ' +
        '주문 facts로 종결 가능하면 고객에게 캡처/추가자료/재문의 등 추가 행동을 요구하지 마라(주문 매칭이 안 될 때만 주문번호/결제내역 확인을 요청). ' +
        '동일 금액 중복 결제 후보가 있거나 환불/취소/반품/교환 완료 여부를 확정할 수 없는 고위험 건일 때만, 고객 초안과 분리해 맨 끝에 "※ 내부 확인 필요: …" 한 줄을 덧붙일 수 있다(이 문구는 고객 초안 안에 섞지 마라).'
    };
  }
