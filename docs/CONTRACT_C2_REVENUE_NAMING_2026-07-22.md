...
`scripts/smoke-c2-revenue-basis-parity-v0.mjs` 기준. **[BASE] 9 pass/0 fail(기준선 불변) · [RED] 0 met/4 unmet(계약 목표).**

| RED | 목표 | 현재 |
|---|---|---|
| A | analytics "매출" 총합 = 유효주문 결제금액 **39,500** | 50,000 (취소·미결제 라인 포함) |
| B | analytics 객단가 = 39,500 ÷ 유효주문수 3 = **13,167** | 10,000 (분자·분모 모두 취소·미결제 포함) |
| C | "매출" 라벨을 **유효주문 결제금액/운영매출**로 명확화 | 맨숫자 `"매출"` |
| D | `netRevenue` 라벨에서 **"순매출" 제거** | `"순매출(라인매출-클레임)"` |

**값 관계(BASE로 잠금)**: 현행 라인기준 매출 50,000 − 계약 유효결제 39,500 = **10,500** = 취소·미결제 라인 **12,000** − 유효주문 배송비·할인 순증 **1,500**. (12,000 ≠ 10,500 인 이유를 값으로 고정.)
