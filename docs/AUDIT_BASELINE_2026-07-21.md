...
| **추가(2026-07-21, RC-1 A-1 중 실증)** | **git-diff 작업범위 가드는 작업 트리가 더러우면 무조건 실패한다.** A-1 수정 중 6개 스모크가 `"src 변경 마케팅 한정"`으로 동시 실패했고 커밋 후 전부 복구됐다(제품 회귀 아님). 수정 전 검증 신호를 오염시키므로 회귀검사와 분리하고, 커밋 시점에만 실행하도록 실행 단계를 나눈다. |
