...
## 11. 후속 대장 (이번 범위 밖 — 기록만)

### DATA-QUALITY-DOMAIN-01 — 품질보고서에 리소스 식별자 없음

`OperationsDataSnapshot.qualityReport` 는 **어느 도메인(orders/inventory/inquiries/reviews/sales)
기준으로 생성된 보고서인지 식별하는 필드가 없다.** `buildOperationsSnapshot` 이 마지막 import
도메인 기준으로 한 벌만 남기기 때문이다.

- **결과**: 주문 자료를 정정해도 그 보고서가 주문 보고서라는 근거가 없다. 마지막 동기화가
  재고였다면 재고 13건짜리 보고서에 주문 유령 1건을 차감해 12건으로 훼손할 수 있다.
- **이번 처리(D-2.1)**: 추정 보정을 하지 않는다. LEGACY 주문 마이그레이션은 `qualityReport` 를
  입력 그대로 보존한다(스모크 G13~G15 로 잠금). 구조 확장·재설계는 하지 않았다.
- **향후**: 리소스별 품질보고서 구조(`qualityReport` 를 도메인 키로 분리)에서 해결한다.
  그때까지 어떤 마이그레이션도 도메인 근거 없이 품질 수치를 계산하지 않는다.
