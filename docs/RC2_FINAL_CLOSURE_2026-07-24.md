...
**현재 최종 운영 기준선** `82878434e253106dbabb9f38e5b01031f0c79096` (BUILD-TYPECHECK-01 후속 설정 보정 병합, 부모 `503e3d8`+`00bc1e0`) = 현재 main = origin/main = Production Source

> **기준선 구분**: RC-2 **기능 구현**의 계보는 `503e3d8`(부모 `6dca85d`+`86513fb`). **현재 최종 운영 기준선**은 후속 빌드 타입검사 보정(BUILD-TYPECHECK-01)을 포함한 `8287843`. RC-2 기능 계보를 `8287843`으로 바꿔 쓰지 않는다.
>
> **정정 이력**: 이 문서는 최초 `7392308`(임시 보존본). 본 정정 커밋은 BUILD-TYPECHECK-01 완료(현재 Production `8287843`, Build-log errors 0)를 반영해 §3·§4.1·§4.7·§6 를 갱신한다. 다른 절의 RC-2 하위 계약 검사 수치는 의미 변경 없이 유지한다.
...
### 3.2 후속 빌드 기준선 복구 (BUILD-TYPECHECK-01) · 현재 운영 기준선
...
| **현재 main = origin/main = Production Source** | `82878434` (`8287843`) — 일반 push, force/amend/rebase 없음 |
| **현재 Vercel Production 배포 ID** | `7baaiGGvq` (`7baaiGGvqadAWbt4QcgvsU2rFoRa`) |
| 배포 상태 | **Ready · Current** (Environment Production), 빌드 32s, Build-log **errors 0 / warnings 1** |
| Production URL | **`godo-psi.vercel.app` HTTP 200** (별칭: `godo-git-main-…`, `godo-96ozfa232-…`) |
...
### 4.1 전체 자동검사 (현재 운영 기준선 main `8287843`, clean tree)
...
- **현재 Production `8287843`(BUILD-TYPECHECK-01)에서 재확인**: `godo-psi.vercel.app` 200, products 13·inventory 13·orders 0·sales 0(api_proxy_real 실제)·inquiries/reviews unavailable — 위 값과 동일(api 소스 무변경이라 런타임 동일). 신규 앱 요청 4xx/5xx 0·신규 콘솔 오류 0.
...
- **현재 Production `8287843`: errors 0 / warnings 1** (Ready·Current)
...
- **BUILD-TYPECHECK-01** — **완료**(§4.7). 현재 Production `8287843` Build-log errors 0.
...
- 본 정정의 변경 파일은 `docs/RC2_FINAL_CLOSURE_2026-07-24.md` **1개뿐**(현재 운영 기준선 `8287843` 을 docs 브랜치에 `--no-ff` 결합 후, 별도 정정 커밋으로 이 문서만 수정).
