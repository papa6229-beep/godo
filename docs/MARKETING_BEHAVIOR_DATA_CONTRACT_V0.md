...
- 다음 단계는 **[GODO Behavior Tracker Script & Collection Endpoint Plan v0](./GODO_BEHAVIOR_TRACKER_COLLECTION_PLAN_V0.md)** — 실제 행동 데이터를 수집하기 전에 *어떤 발자국을 어떤 모양으로 안전하게 받을지* 그리는 바닥 설계 문서가 추가되었다.
- 그 설계의 코드 계약 상수는 `src/services/marketingBehaviorCollectionPlan.ts`에 단일 소스로 둔다(추적 이벤트·source 규칙·data attribute·세션/구매 정책·PII 금지·향후 endpoint 한도).
- **route/API는 아직 만들지 않는다** — `/api/marketing/behavior-events`는 "향후(미생성)"로만 언급된다.
- 현재 모달은 **demo / live 모두 동일한 `MarketingBehaviorInsights` 계약**을 사용하므로, 수집이 시작되면 모달 호출 인자만 `{ mode: 'live', fallbackDemo: false }`로 바꾸면 된다.
- 전체 흐름: **`MarketingBehaviorEvent` → (수집/저장) → `aggregateMarketingBehaviorPatterns()` → `convertAggregatedPatternToInsights()` → `MarketingBehaviorInsights` → 모달**. 집계 builder 문서: [MARKETING_BEHAVIOR_AGGREGATED_PATTERN_BUILDER_V0.md](./MARKETING_BEHAVIOR_AGGREGATED_PATTERN_BUILDER_V0.md).
