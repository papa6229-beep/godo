...
**① 지금 업무 기록은 전부 한 사람의 브라우저 안에만 있다.** 다섯 영역이 localStorage 키 5개에 들어간다. 다른 직원의 브라우저에는 그 기록이 존재하지 않는다.
...
**⑤ 여러 사람이 동시에 쓰면 서로의 기록을 덮어쓴다.** 모든 쓰기가 “전체 목록을 읽어 → 고쳐 → 전체를 다시 저장”이라서, 두 사람이 거의 같은 때 저장하면 나중 저장이 앞 저장을 지운다. 서버 저장에서는 **행 단위 저장(append/upsert)** 으로 바꿔야 한다.
...
모든 쓰기가 **전체 목록 재작성**이다 → 동시 저장 시 **나중 저장이 앞 저장을 덮어쓴다(lost update)**.
