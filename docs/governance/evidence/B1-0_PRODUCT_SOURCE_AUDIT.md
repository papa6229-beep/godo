...
### 13건의 출처 = **기존 시험몰 (실제 외부 Open API 응답)**

`sourceType: api_proxy_real`은 **정확하다.** 시뮬레이션·fixture·mock이 주입된 것이 아니다.
...
**`MISS` + `age:0` + 1.8초**는 CDN 캐시나 이전 배포 재사용이 아니라 함수가 새로 실행되어 **외부 왕복을 수행**했음을 뜻한다.
...
| 후보 | 판정 | 근거 |
|---|---|---|
| 실제 외부 고도몰 API 성공 | ✅ **이것** | 분기 3만이 이 라벨을 붙임 · `x-vercel-cache: MISS` · 1.8~3.0초 왕복 · 같은 시점 `unavailable`과 구분됨 |
