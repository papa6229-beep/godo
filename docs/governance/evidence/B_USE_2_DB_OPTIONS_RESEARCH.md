...
| 12 | 실시간 반영 수단(구독/폴링) | 우리가 구현(LISTEN/NOTIFY 등) | **제품 기능 있음** — 가격표에 `Realtime / Postgres Changes` **"Included"** · 동시 최고 연결 Free **200** · Pro·Team **500 포함** 후 $10/1000 (S1) | **전용 제품은 미확인.** 다만 **풀링 transaction mode 에서는 `LISTEN`/`NOTIFY` 가 지원되지 않으므로**(N3) 직접 연결이 필요하다는 점은 확정 | 부분적 |
...
| 항목 | 초판 | 1차 교정 | **2차 교정(현재)** |
|---|---|---|---|
| 공식 출처 개수 | 16 | 21 | **28** (S1~S12 · N1~N13 · P1~P2 · V1) |
| 확정된 사실 | 15 | 20 | **27** |
| 조건부 사실 | 3 | 3 | **4** |
| 미확인 항목 | 31곳(낱말 수 기준, 부정확) | 21개 | **17개**(개별 7 + 회사 서버 확인표 10) |
...
| 5 | Neon **실시간 반영 수단** | Neon | A |
| 6 | **무료 등급 적합 여부**(실제 사용량 부재 — §1-②) | 셋 다 | A(우리 관측값 부재) |
| 7 | **Neon 직접 연결 상한의 실제 값** — 공식이 컴퓨트 구성에 종속되고 정본은 Console/`SHOW max_connections` 인데 **우리 프로젝트가 없다** | Neon | A(프로젝트 부재) |
| 8~17 | 회사·고도몰 서버 **확인표 10개 항목 전부**(§5) | 직접 운영 | A(사용자 입력 대기) |
