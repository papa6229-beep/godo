...
작성일 **2026-08-21** · **교정 2026-08-21**(§7 전면 교체 · §4 A의 뜻 명확화 · §1-3 진입점 사실 보강 — 사유는 §7) · 기준 HEAD `47088fc` (브랜치 `codex/d0-parallel-team-readiness-plan`) · main `8ad9557` · origin/main `5190f685`
...
**A의 뜻을 먼저 못박는다.** A는 **"Vercel 함수에 단순형을 올린다"가 아니다.** A는 **"Vercel 에 있는 GODO 화면이, 별도로 상시 떠 있는 Python 서비스 1대를 호출한다"** 는 뜻이다. 단순형은 Vercel 배포 산출물 안으로 들어가지 않는다.
...
## 7. 다음 단계의 선행 결정 — **하나뿐이다**

> **GODO Vercel 화면이 호출할 상시 Python 서비스의 실행 장소를 사용자와 결정한다. 서비스는 HTTPS, 접근 통제, 지속 작업 디스크를 가져야 하며 단순형 정본 26파일의 변환 규칙은 변경하지 않는다.**

이 결정이 서기 전에는 §5의 경계도, 화면 쪽 주소 계약도 설계하지 않는다.

### 이전 판(2026-08-21 초판)에서 제거한 것과 그 이유
...
### 이 결정이 선 뒤에 잴 두 숫자 (지금은 미확인 — §6-3·6-4)

상품 1건 변환 wall-time(초) · 결과 HTML 바이트. **서비스가 실제로 서는 그 환경에서 잰다.** 다른 환경에서 잰 값을 그 환경의 값으로 쓰지 않는다.

**어느 경우에도 하지 않는 것**: 정본 26파일 수정 · GODO 함수 추가 · 새 저장소·DB·Blob 생성 · 환경변수 변경 · Production 배포 · 실상품 대량 변환.
