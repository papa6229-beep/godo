...
>
> **두 흐름이 동시에 열려 있다 (D-009).** 하나가 다른 하나를 멈추지 않는다.
>
> | 흐름 | 상태 | 지금 하는 일 |
> |---|---|---|
> | **C — 실제 연결 검증** | **키 확보 완료 · READ 연결 검증 통과 · 계약 실측 대기** | 신규 실몰 키로 **상품·주문 READ 연결과 접근 권한을 실증했다**(2026-08-04, 로컬 `.env.local`). **주문 필드·매출·결제완료·취소 계약은 실주문 0건이라 미검증**이며, 신규 몰에 시험 주문이 들어오는 즉시 같은 경로로 이어서 검증한다(§6). 관측값은 `CURRENT_STATE.md §3` |
> | **D-0 — 팀 기능 준비** | **진행 가능** | 고도몰 스펙에 맞춘 **기존 시험 데이터**로 상품·CS·디자인·마케팅·HQ 기능을 계속 만들고 자동검사한다 |
>
> **키가 없다는 이유로 다른 개발 전체를 무기한 중단하지 않는다.** 반대로, 시험자료 기반 자동검사를 **실제 데이터 실증으로 승격하지도 않는다**(헌법 §10). 두 가지는 다른 증거다.
> **가상 데이터는 계속 `시험 데이터` 로 표시한다.** 실제 개인정보 저장·외부 WRITE·Production 변경의 승인 경계는 그대로다.
> **C 가 실주문을 기다리는 중에도 B 를 다시 열지 않는다.** 승인된 `Local migration`(§5-4) 경로의 독립 작업도 계속할 수 있다.
...
> **D-0 완료 (2026-07-30, 브랜치 `codex/d0-parallel-team-readiness-plan`, 문서 전용)**: 팀 기능 준비도 조사와 첫 대표 업무 선정. 산출물 `docs/governance/evidence/D0_PARALLEL_TEAM_FUNCTION_READINESS.md`.
> **추천한 다음 대표 업무 1개**: 상품팀 **재고위험 일일 점검 → 팀장 승인 → HQ 보고**(`src/data/defaultAgentTasks.ts` 의 `task-product-daily`). 시험 데이터로 지금 구현·검증할 수 있고, 실제 상품이 등록되면 **입력만 바뀌어** 같은 경로로 재시험된다. 개인정보·외부 WRITE·DB 선택을 선행 요구하지 않는다.
> 관측된 남은 간극(추정 아님): `runScheduledAgentTask` **제품 호출자 0건**(예약 실행은 E) · 활동 원장 사람 라벨 `'운영자'` 하드코딩(`agentTaskRunner.ts:74,117`) · 보고와 HQ 승인 흐름의 연결.
> **D-0 첫 대표 업무 로컬 구현 완료 (2026-07-30, Codex 독립검증 대기)**: 위 추천 업무 `task-product-daily` 를 **팀 내부 완주**로 구현했다(근거 결정 **D-010** — HQ 자동 보고 경계).
> 상품팀장이 시험자료로 실행 → 결과에 `시험 데이터` 표시 → 팀장이 `확인 완료` 또는 **이유 한 문장으로 반려** → 결과·상태·행위자·사유가 **새로고침 뒤에도 복원**된다(상태 정본 = append-only `activityLedger`). **HQ 메시지·HQ 승인대기는 자동 생성하지 않고**, HQ 는 기존 `오늘의 운영 → 팀 카드 → 부서 업무 확인` 에서 열람한다. `'운영자'` 하드코딩 3곳을 제거하고 실제 로그인 행위자(`label`+`userId`)를 기록한다. 데이터 미연결은 완료·대기로 저장하지 않고, pending 중 재클릭은 중복 기록을 만들지 않는다.
> **HQ 가 직접 지시한 기존 업무 흐름(B-use)은 변경하지 않았다.** `taskLifecycleAppAdapter` 에 이 반복 업무를 등록하지 않았다. 기존 검사 확장(**55/55**) · 신규 smoke 파일 0 · **manifest 125/0 불변**.
> **전체 `npm test` 는 Claude 가 실행하지 않았다 — Codex 독립검증 대기.**
> **이것은 시험자료 기반 미리 구현 완료다. 고도몰 실데이터 검증(C) 완료도, 전체 팀 기능 완료도 아니다.** 실제 상품·실제 데이터로 재시험하기 전에는 E·F 의 실제 완주로 표시하지 않는다. **고도몰 키 발급과 실제 상품 준비는 계속 병렬 대기다.**
> **Codex 전체 게이트 1차 (기준 `db5ec25`)**: 제품 집중검사 `smoke-agent-task-runner-v0` **55/55** · build · API typecheck · 전체 lint **통과** · 구현/문서 커밋 `git show --check` 통과. 다만 **전체 smoke 는 121/125(136.8초) · 실패 4건**이었다.
> **실패 4건은 제품 결함이 아니라 옛 검사 기준이었다** — `d13` W21(옛 변수명 `viewerRole` 만 탐색) · `d131` S25(공개 함수 소스 400~600자 거리 정규식, 권한 검사가 공통 helper 로 이동해 실패) · `task-lifecycle` R2a·R7b·R7c(옛 호출 계약: actor 에 `accountRole` 없음·`revenue:null`·`approveAgentTask` 옛 인자 순서) · `d121` R4(위 S25 를 다시 호출한 **연쇄 실패**).
> **검사 정합 교정 완료(2026-07-30) — 제품 코드 변경 0건.** W21·S25 는 문자열 검사에서 **실제 반환값 실행 검사**로 바뀌어 오히려 강해졌고, R2a/R2b/R2c·R7b/R7c 의 원래 목적은 낮추지 않았다. 지정 검사 5종 전부 exit 0 · manifest 125/0 불변.
> **→ Codex 전체 게이트 통과 (기준 HEAD `d26019a`, 직접 실행)**: `npm test` **exit 0** · smoke **125/125 · 139.2초** · `tsc -b` · API typecheck · Vite build · 전체 lint · `git diff --check main..HEAD` 통과 · manifest **125/0** · 작업 트리 clean. Vite 의 큰 번들 경고는 **기존 비차단 경고**이며 이번 업무의 실패가 아니다.
> **D-0 첫 대표 업무 `task-product-daily` 최종 확정 — 범위는 다음과 같다.**
> ✅ **시험자료 기반 미리 구현·자동검증 완료** · 상품팀 내부에서 팀장이 실행 → 결과 확인 또는 이유 있는 반려 → 기록 복원 · **HQ 메시지·HQ 승인대기 자동 생성 없음**
> ❌ **실제 상품·실데이터 재시험 미완료** · **C 완료 아님 · E·F 실제 완주 아님 · 전체 팀 기능 완료 아님**
>
> **D-0 두 번째 팀 업무 로컬 구현 완료 (2026-07-30, Codex 독립검증 대기)**: **CS 문의·리뷰 일일 점검 팀 내부 완주**(`task-cs-daily`). D-010 을 그대로 적용해 `reportTo: 'hq' → 'cs'` 로만 바꿨다(`approvalMode: 'draft'` 유지 — AI 초안을 CS팀장이 수정해 확정한다). **새 실행기·저장소·승인 체계 0건 · 팀 이름 조건문 0건** — 상품팀에서 만든 일반 경계 `reportTo === teamId` 를 그대로 재사용했다. 예약 실행은 연결하지 않았다.
> 기존 `focus:'cs'` 계산(총 문의·미처리·리뷰·자동응대 후보)을 그대로 쓰고 **새 계산식·문의 상태 재판정 0건**이다. 시험자료는 `시험 데이터` 로 표시되고, `unavailable`·데이터 없음은 기록·메시지 0건이다.
> **CS real 문의·리뷰 경로가 없다는 사실은 실제 데이터 최종 실증을 막을 뿐, 시험자료 기반 구현을 막지 않는다**(D-009). **CS 실제 고객 답글 발송은 계속 미연결·범위 밖**이다.
> 기존 검사 확장(**69/69**) · 신규 smoke 파일 0 · **manifest 125/0 불변** · 상품팀 무회귀 유지.
> **→ Codex 전체 게이트 통과 (기준 HEAD `6a23d02a`)**: `npm test` **exit 0** · smoke **125/125 · 131.2초** · `tsc -b` · API typecheck · Vite build · 전체 lint 통과 · manifest **125/0** · 작업 트리 clean.
> **CS팀 `task-cs-daily` 확정 — 범위**: ✅ 시험자료 기반 미리 구현·자동검증 완료 · CS팀장이 AI 초안을 수정해 팀 내부 마감 · **HQ·CS 요청함 자동 생성 0건** / ❌ **실제 문의·리뷰 API 와 고객 답글 WRITE 미연결** · 실제 데이터 최종 실증 아님 · 전체 CS 기능 완료 아님. **자동검사 통과이며 Preview·Production 화면 확인으로 확대하지 않는다.**
>
> **D-0 세 번째 팀 업무 로컬 구현 완료 (2026-07-30, Codex 독립검증 대기)**: **마케팅 일일 매출 요약 팀 내부 마감**(`task-marketing-daily`). `reportTo: 'hq' → 'marketing'` · `approvalMode: 'auto' → 'approval'`.
> **착수 전 이견 → Codex A안 판정**: 최초 지시의 `auto` 유지 + 즉시 완료는 현재 계약과 양립하지 않았다. `task-marketing-daily` 에 승인된 `standing` 이 없어 `canRunStandingDirective(undefined)` 가 `requiresLeadConfirmation:true` 를 돌려주기 때문이다(`standingDirectiveContract.ts:63-71` · `agentTaskRunner.ts:292-295`). **승인받은 적 없는 `standing` 을 만들지 않고, 공통 안전 경계도 바꾸지 않았다.** 대신 상품·CS 와 같은 흐름(팀장 확인 후 팀 내부 마감)으로 맞췄다.
> **`매일 09:30` 은 업무 설정값이고 실제 시각 자동 스케줄러는 미연결이다**(`runScheduledAgentTask` 제품 호출자 0건 — E 단계). 현재는 **마케팅팀장이 직접 실행하고 확인**하며, 승인된 standing 이 없으므로 **스케줄 자동 실행은 계속 차단**된다.
> 기존 `focus:'sales'` 계산(운영매출·운영주문·객단가·기간) 그대로 · **새 매출 정의 0건** · 새 실행기·저장소 0건 · 마케팅 전용 조건문 0건. 기존 검사 확장(**86/86**) · 신규 smoke 0 · **manifest 125/0 불변** · 상품팀 12건·CS팀 14건 무회귀.
> **마케팅 고객 행동수집 미연결은 실제 마케팅 실증을 막지만, 기존 매출 시험자료 요약 구현은 막지 않는다**(D-009).
> **이번 마케팅 업무는 시험자료 기반 내부 자동 기록이며 실제 마케팅 기능 완주도, E·F 완료도 아니다. 고도몰 키는 발급 대기 중이다.**
> **Codex 전체 게이트 1차 (기준 `95bc2f1`)**: 마케팅 집중검사 전부 통과(runner 86/86 · revenue parity 20/20 · provenance 42/42 · marketing facts 34/34)했으나 **전체 smoke 124/125 · 실패 1건**(`smoke-agent-task-store-v0.mjs` 가 옛 기대값 `마케팅=auto` 유지). smoke 실패로 build·lint 는 미실행. 최초 `npm test` 는 **PowerShell `npm.ps1` 실행 제한**으로 시작조차 못 했고(코드 실패 아님) `npm.cmd test` 가 실제 결과다.
> **후속 교정 완료 (2026-07-30, Codex 독립검증 대기)**: 검사 기대값보다 큰 실제 결함이 드러났다 — `agentTaskStore.loadAgentTasks()` 가 저장값이 있으면 기본 스펙을 보지 않아, **이미 앱을 쓴 브라우저에는 옛 정책(`reportTo:'hq'`·마케팅 `auto`)이 그대로 남았다.** 기본 업무 **보고 정책 1회 이관**(marker `godo_agent_tasks_policy_v1`)을 추가했다. **id 가 같은 기본 업무 3종의 `reportTo`·`approvalMode` 만** 고치고, 사용자 추가 업무·삭제된 기본 업무·정책 외 필드·업무 결과/원장/승인 이력은 손대지 않는다. marker 이후에는 Studio 수정값을 덮어쓰지 않는다. 기존 검사 확장(8 → **19/19**) · runner **86/86** 무회귀 · 신규 smoke 0 · manifest **125/0 불변**.
> **→ Codex 전체 게이트 통과 (기준 `7f3504c`)**: 저장 이관 집중검사 19/19 · 업무 실행 무회귀 86/86 · 전체 smoke **125/125 · 153.4초** · `tsc -b` · API 타입검사 · Vite build · 전체 lint · `git diff --check main..HEAD` 통과 · 트리 clean. (첫 호출은 **실행 제한 1초 오설정**으로 시작 직후 종료 — 코드 실패 아님.)
> **마지막 빈틈 교정 완료 (2026-07-30, Codex 독립검증 대기)**: 저장목록이 없을 때 현재 기본값을 내주면서 **marker 를 남기지 않아**, 새 브라우저에서 사용자의 **첫 설정 수정이 다음 로드에 되돌아가는** 빈틈이 있었다(손상 저장값 복구 경로 2곳도 동일). 현재 기본값을 그대로 내주는 세 경로를 하나로 모아 **그때 marker 를 남기도록** 최소 수정했다 — 새 버전 체계·저장소 개편 0건. 기존 이관 동작·사용자 추가 업무·삭제한 기본 업무·정책 외 필드·업무 결과/원장/승인 이력은 무변경. 검사 확장(19 → **25/25**) · runner **86/86** 무회귀 · manifest **125/0 불변**.
> **→ Codex 최종 전체 게이트 통과 · D-0 저장 정책 교정 최종 확정 (기준 HEAD `63412ae`)**: 저장 경계 집중검사 **25/25** · 업무 실행 무회귀 **86/86** · `npm.cmd test` **exit 0** · 전체 smoke **125/125 · 114.3초** · manifest include **125**/exclude **0** · `tsc -b` · API 타입검사 · Vite build · 전체 lint · `git diff --check main..HEAD` 통과 · 트리 clean.
> **확정 항목**: 기존 브라우저 옛 정책 1회 이관 · 새 브라우저 첫 Studio 수정 보존 · 손상 JSON·비배열 복구 뒤 첫 수정 보존 · 저장 실패 시 marker 미생성과 복구 후 재시도 · 사용자 추가 업무/삭제한 기본 업무/정책 외 필드 보존 · 업무 결과·활동 원장·승인 이력·메시지 무변경 · **상품·CS·마케팅 일상 업무는 팀 내부 마감** · **HQ 요청함·HQ 승인대기 자동 생성 없음** · HQ 지시 업무와 팀장 자율 보고 경로는 별도 유지.
> **이것은 시험자료 기반 미리 구현·자동검증 완료다. 실제 고도몰 데이터 실증도, E·F 실제 업무 완주도 아니다.** 화면 동작을 바꾼 작업이 아니므로 Preview·Vercel·브라우저 검사는 하지 않았고 화면 실증으로 확대하지 않는다.
>
> **다음 한 작업: 디자인팀의 첫 대표 업무는 사용자 의도 확인 뒤 확정한다.** 현재 추천 후보는 `시험상품 상세페이지 초안 생성 → 디자인팀장 수정·확인 → 팀 내부 저장` 이며, **고도몰 등록·외부 WRITE 는 포함하지 않는다.**
> **위 "키 확보 시 C 우선" 조건은 2026-08-04 에 발동돼 이행됐다** — 사용자 승인 아래 C 의 연결 검증을 먼저 수행했고(상품·주문 READ), 그 결과 **C 의 남은 종료조건은 코드가 아니라 실주문 유무에 막혀 있다.** 사용자는 **지금 시험 주문을 만들지 않기로** 했다. 따라서 **다음 한 작업은 위 디자인팀 항목 그대로**이며, 시험 주문이 준비되면 C 를 다시 우선한다.
> **DB 공급자 선택이나 서버 어댑터 구현을 다음 작업으로 만들지 않는다.**
> **현재 단계 C 는 계속 열려 있다 — 연결·권한은 검증됐고 필드·계약 실측은 실주문 부재로 미착수다. B-use 를 다시 열지 않는다.**
>
> ---
>
...
- **키 확보 즉시** 연결 상태·인증·공식 응답 구조·출처 판정·실제 0건/미연결 구분부터 검증한다.
- **완성된 실제 판매상품 카탈로그는 C 연결 착수 조건이 아니다.** 상품 선별·상세페이지 변경은 상품팀 실무이며 그것을 기다려 연결 검증을 미루지 않는다.
- 아직 실제 사례가 없는 **주문·문의·리뷰·클레임 필드는 `실측 대기`** 로 남긴다. **가상자료 결과를 실제 관측으로 승격하지 않는다**(헌법 §10).
- **실제 상품과 시험 주문이 들어오는 즉시 같은 계약을 재검증한다.** 재검증은 새 작업이 아니라 같은 종료조건의 두 번째 증거다.
...
**진행 조건 (D-009)**: **C 의 키 발급이나 상품팀의 카탈로그 준비를 이유로 D→E→F 준비를 전부 멈추지 않는다.** D→E→F 의 **내부 순서는 유지**하되, 고도몰 스펙에 맞춘 **기존 시험 데이터**로 진행할 수 있다. 이때의 자동검사는 **시험자료 기반 검증임을 함께 적는다.**

### D-0 — 팀 기능 준비도 조사 (2026-07-30, 문서 전용)

산출물: `docs/governance/evidence/D0_PARALLEL_TEAM_FUNCTION_READINESS.md`
상품·CS·디자인·마케팅·HQ 별로 ① 지금 진입 가능 ② 시험 데이터로 개발·검사 가능 ③ 새 키만 있으면 확인 가능 ④ 실제 상품 등록 필요 ⑤ 실제 고객정보 필요 ⑥ 외부 WRITE·사용자 결정 필요 를 **현재 코드와 마운트 경로 기준**으로 갈랐다.
**추천한 다음 대표 업무 1개**: 상품팀 **재고위험 일일 점검**(`src/data/defaultAgentTasks.ts` 의 `task-product-daily`).

**→ 2026-07-30 구현 완료(로컬) · Codex 독립검증 대기.** 다만 최종 형태는 **HQ 보고가 아니라 팀 내부 완주**다 — 사용자 결정 **D-010**(HQ 자동 보고 경계)에 따라 `팀장 확인 완료 또는 이유 있는 반려`로 팀 안에서 마감하고, HQ 는 기존 부서 업무 확인 화면에서 **열람**한다. 자세한 내용은 `CURRENT_STATE.md`.
...
**두 단계로 나눈다 (D-009)**: **① 미리 구현·시험** — 시험 데이터로 계획→실행→비용기록→결과→승인→원장을 만들고 자동검사한다. 키·실제 상품을 기다리지 않는다. **② 실제 데이터 최종 실증** — 실제 상품·시험 주문이 준비된 뒤 같은 경로를 실제 데이터로 완주한다. **①만으로 E 를 완료로 표시하지 않는다.**

## 9. 단계 F — 팀별 대표 업무 구현·확대

모든 팀을 동시에 만들지 않는다. 선정 기준: 실제 데이터 준비 · 팀장 절차 확인 · 사용자 가치 · 위험 · 외부 의존 준비

**선정 기준 보완 (D-009)**: "실제 데이터 준비"는 **착수 조건이 아니라 최종 완료 조건**이다. 실제 상품이 없어도 **시험 데이터로 먼저 구현·검증할 수 있는 업무를 우선 선정**하고, 실제 상품·시험 주문이 들어오면 **같은 경로로 재시험**해 완주를 확정한다. **미리 구현·시험 가능**과 **실제 데이터 최종 실증 완료**는 다른 상태이며 완료 표기를 섞지 않는다.
...
- 단계 C: 키·실데이터 확보 후 재산정. **키 대기 기간은 다른 흐름의 일정에서 빼지 않는다** — D-0 는 그 기간에 진행한다(D-009)
- 단계 D 이후: 팀별 작업 카탈로그 전에는 총기간을 약속하지 않음

각 묶음은 착수 전에 범위와 예상 일수를 제시한다. **일정 때문에 검증을 생략하지 않는다**(헌법 §13).

**대기와 정지를 구분한다 (D-009)**: C 가 키를 기다리는 것은 **그 흐름의 대기**이지 프로젝트 전체의 정지가 아니다. 같은 기간에 시험 데이터 기반 팀 기능 준비가 진행되며, **실제 데이터 실증만 뒤로 미뤄진다.**
