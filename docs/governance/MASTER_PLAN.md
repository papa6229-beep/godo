...
> **다음 한 작업: Codex 가 DB 후보 2~3개의 공식 가격·운영 조건을 조사해 사용자 추천안을 만든다.**
> 그 입력이 되는 저장소 기술 사실은 `docs/governance/evidence/B_USE_2_SERVER_RECORDS_WORKLOAD.md` 에 정리됐다(브랜치 `codex/b-use-2-server-records-decision-input`, 2026-07-28).
> **DB 가 정해지기 전에는 서버 어댑터를 구현하지 않는다.**
