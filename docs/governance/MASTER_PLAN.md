...
> **다음 한 작업: Local migration 6 — 미사용 고도몰 상품 매퍼 제거**(`api/_shared/godomallMapper.ts` 의 `ProductIntermediate`·`InventoryIntermediate`·`mapGoodsList`·`mapGoodsToInventory` 4종. 근거 없는 `safetyStock` 기본값 `'5'` 도 함께 사라진다). 활성 경로 `mapGoodsToProducts`·`deriveInventoryFromProducts` 는 보존한다.
