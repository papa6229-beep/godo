...
> | 흐름 | 상태 | 지금 하는 일 |
> |---|---|---|
> | **C — 실제 연결 검증** | **키 발급 대기** | 새 판매몰 개발자 등록·API 키를 기다린다. 키가 확보된 사실이 확인되면 사용자가 언급하지 않아도 **즉시 C 착수를 제안하고 연결 검증을 병행**한다(§6) |
...
> **키 대기 중에도 B 를 다시 열지 않는다.** 승인된 `Local migration`(§5-4) 경로의 독립 작업도 계속할 수 있다.
