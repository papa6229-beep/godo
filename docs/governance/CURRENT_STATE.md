...
**삭제 전 선언·전달·실사용 관계 (직접 확인)**

| prop | 선언 | 전달 | 구조분해 | 본문 사용 |
|---|---|---|---|---|
| `CalendarPanel.activeOperationsData` | `CalendarPanel.tsx:14` | `MainLayout.tsx:567` | **없음** | **0건** |
| `TeamOperationsBoard.lastRunJobs` | `TeamOperationsBoard.tsx:18` | `OfficeView.tsx:128` | **없음** | **0건** |
| `TeamOperationsBoard.onApprove` | `:30` | `OfficeView.tsx:140` | **없음** | **0건** |
| `TeamOperationsBoard.onReject` | `:31` | **없음** | **없음** | **0건** |

달력은 자체 `fetchRevenue`(`CalendarPanel.tsx:89`) 결과를 쓴다 — `activeOperationsData` 는 값만 흘러들어오고 어디에도 닿지 않았다.

**제거한 것**: 위 4개 prop 선언 · `CalendarPanel` 의 그 prop 전용 `OperationsDataSnapshot` 타입 import · `TeamOperationsBoard` 의 그 prop 전용 `AgentJob` 타입 import(파일 내 유일 사용처였다) · `MainLayout` 의 `<CalendarPanel activeOperationsData={...}>` 전달 · `OfficeView` 의 `<TeamOperationsBoard>` 로 가던 `lastRunJobs`·`onApprove` 전달.

**보존한 것(직접 확인)**: `MainLayout` 의 **다른** `activeOperationsData` 사용 · `CalendarPanel` 의 `fetchRevenue` 경로 · `TeamOperationsBoard` 의 `approvalItems`·`onOpenApprovals`(승인 목록 진입) · `lastRunResults`·`lastRunHandoffs`(실제 사용 중) · `OfficeView` 자신의 `onApprove` prop 과 **다른 활성 승인 화면 2곳으로 가는 전달**(`OfficeView.tsx:154`·`:185`) · `pendingApprovalsForIdentity` · 승인 목록 진입·확인 완료·반려 흐름.

**RED → GREEN** (기존 `smoke-rc2-app-integration-red-v0.mjs` 에 최소 단언 추가 · 신규 smoke 0 · manifest **125/0 불변**)

| | RED (삭제 전 실제 출력) | GREEN |
|---|---|---|
| M1 CalendarPanel 미사용 계약 부재 | **RED** — 잔존 `OperationsDataSnapshot, activeOperationsData` | MET |
| M2 MainLayout 전달 부재 | **RED** — CalendarPanel 전달 잔존 | MET |
| M2a MainLayout 의 다른 사용 유지(과삭제 방지) | RED 단계에서도 MET(안전망) | MET |
| M3 CalendarPanel `fetchRevenue` 유지 | RED 단계에서도 MET(안전망) | MET |
| M4 보드 미사용 prop 3종 부재 | **RED** — 계약 잔존 `lastRunJobs, onApprove, onReject` · OfficeView 전달 잔존 `lastRunJobs, onApprove` | MET |
| M5 활성 승인 경로 유지 | RED 단계에서도 MET(안전망) | MET |
...
**Claude 가 실행하지 않은 것**: 전체 `npm test` · Preview·Vercel·브라우저 · main 통합·push·배포·환경변수·외부 WRITE.

##### → Codex 최종 전체 게이트 통과 · **D-0 저장 정책 교정 최종 확정** (기준 HEAD `63412ae`, 2026-07-30)
...
|---|---|
| 저장 경계 집중검사 | **25/25** |
| 상품·CS·마케팅 업무 실행 무회귀 | **86/86** |
| `npm.cmd test` | **exit 0** |
| 전체 smoke | **125/125 · 114.3초** |
| manifest | include **125** / exclude **0** |
| `tsc -b` · API 타입검사 · Vite build · 전체 lint | 전부 통과 |
| `git diff --check main..HEAD` | 통과 |
| 작업 트리 | clean |

**최종 확정 항목**

- ✅ 기존 브라우저의 **옛 기본 업무 정책 1회 이관** 완료
- ✅ **새 브라우저의 첫 Studio 수정 보존** 완료
- ✅ **손상 JSON·비배열 복구 뒤 첫 수정 보존** 완료
- ✅ **저장 실패 시 marker 미생성 · 저장 복구 후 재시도** 완료
- ✅ 사용자 추가 업무 · 사용자가 삭제한 기본 업무 · **정책 외 필드 보존**
- ✅ 업무 결과 · 활동 원장 · 승인 이력 · 메시지 **무변경**
- ✅ 상품·CS·마케팅 **일상 업무는 팀 내부 마감**
- ✅ **HQ 요청함·HQ 승인대기 자동 생성 없음**
- ✅ HQ 가 지시한 업무와 팀장이 자율적으로 보내는 보고 경로는 **별도 유지**

**이것은 시험자료 기반 미리 구현·자동검증 완료다. 실제 고도몰 데이터 실증도, E·F 실제 업무 완주도 아니다.**
**화면 동작을 바꾼 작업이 아니므로 Preview·Vercel·브라우저 검사는 하지 않았고, 이를 화면 실증으로 확대하지 않는다.**
