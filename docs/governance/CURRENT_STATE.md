...
| 현재 작업 브랜치 | `codex/b-use-5-preview-acceptance` (**main 미통합**) — B-use-5 Preview 인수검사 결함 7건 마감 + Codex 지적 4건 교정 | `git rev-parse --abbrev-ref HEAD` |
...
### B-use-5 Preview 인수검사 — **사용자 실제 화면 1회 수행 · 결함 7건 마감 · Codex 2차 독립검증 통과 · Preview 갱신과 사용자 화면 재확인 전 (브랜치 `codex/b-use-5-preview-acceptance`, 기준 `bcf91a4`, 2026-07-28)**

> **이것은 완료 선언이 아니다.** Codex 독립검증은 통과했지만 **7건이 화면에서 실제로 고쳐져 보이는지는 아직 사용자가 확인하지 않았다.** B-use-5 는 완료가 아니다.
...
- **실제 브라우저 화면** — 7건이 사용자 눈에 실제로 고쳐져 보이는지는 **사용자 Preview 재확인에서만 판정**한다. 자동검사는 배선과 계약을 증명할 뿐 화면 관측이 아니다.
- **Codex 독립검증은 완료**됐다(2차, 기준 `bcf91a4`). 남은 것은 **Preview 재배포 후 사용자 화면 재확인**이다.
- **main 미통합 · Production 미배포 · 환경변수 무변경.**
...
- **한국 리전**: Supabase **서울 `ap-northeast-2` 있음** · Neon **없음**(리전 변경도 불가). Supabase 공식 문서는 **"선택한 지역 안에 모든 데이터가 남는다"** 고 명시하며(지역 선택·읽기 복제본 지역은 고객 책임), 이는 **기술적 저장 지역 사실**이지 국내 보관 법적 의무 충족을 뜻하지 않는다. CS 완료 기록에 PII 그릇이 있으므로 C단계 실데이터 연결 이후 실제 조건이 된다.
...
**여전히 미확인**: 무료 프로젝트 자동 삭제 규정(양쪽) · Neon 백업 방식·IPv4/IPv6·저장 지역·at-rest 암호화·Object Storage 가격·실시간 수단 · Supabase 리전 변경 가능 여부·at-rest 암호화 표준 · Vercel 목록의 Supabase 네이티브 통합 여부 · **무료 등급 적합 여부** · 회사 서버 확인표 10항목.

**DB 는 여전히 미결정이다.** 표준 PostgreSQL 공통분모 권고는 그대로 유지한다.
